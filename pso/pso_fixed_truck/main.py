...
def objective_function(particles, car_particle):
    ...
    for i in range(numofcar):
        no_vehicle = np.where(car_particle==i)[0]
        total_dist = dist[0,particles[no_vehicle[0]]]
        for j in range(len(no_vehicle)-1):
            start = particles[no_vehicle[j]]
            end = particles[no_vehicle[j]+1]
            total_dist = total_dist + dist[start,end]
        total_dist = total_dist + dist[end,0]
        fitness= fitness+total_dist
    return int(fitness)
...
dist = pd.read_csv('matrix.csv', header=0, index_col=0)
dist = dist.values
